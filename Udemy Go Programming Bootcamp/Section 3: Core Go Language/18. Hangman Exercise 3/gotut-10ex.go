...
func showBoard() {
	...
	fmt.Println("Hangman state :", hmArr[len(wrongGuesses)])

	fmt.Print("Secret Word : ")

	// Prints spaces for missing letter or the letter
	// Cycle and if is a character print it and if not, print an underscore
...
func getUserLetter() string {
	...
	for true {
		...
		// Create regular expression that verifies if a single character is a letter
...
func getAllIndexes(theStr, subStr string) (indices []int) {
	...
	// Offset will find 1st matching index and then move forward to find the rest
	offset := 0
	for {
		i := strings.Index(theStr[offset:], subStr)

		// If no matches or no more matches since we cycled to the end
		if i == -1 { // no more matches
			return indices
		}
		offset += i
...
func updateCorrectLetters(letter string) {
	...
	// Assign all matching indexes the supplied letter
...
func main() {
	fmt.Println("The random word is ", getRandWord())
	...
	for true {
		...
		if strings.Contains(randWord, guess) {
			...
			// They guessed a letter not in the secret word and append to string
			...
			if len(wrongGuesses) >= 6 {
				fmt.Println("Sorry You'r Dead! The word is", randWord)
				break
			}
		}
	}
